...
// ]
const quantity=30;
export default class Media_img_muti extends Component {
    constructor (props) {
        super(props)
    ...
    // 
    render() {
        let {selected,img_bs,selected_show_img,title,img_list,show_more,selected_ban_sao,lock,show_more_bs,text_check}=this.state;
            ...
            >
              ...
              <Modal.Body>
                <div className='row bozy'>
                    <div ref={this.myRef_scroll}></div> 
                    {
                        imgs.map((item,i)=>{
                            let check=false;
                            if(text_check.search(selected_show_img+item.id+",")>-1) check=true
                            return (
                                <div className='col-3 card-imgz' key={item.id}>
                                    <div className={check?'row wrap-imgz active':'row wrap-imgz'}>
                                        <div className='col-12 img-ak'
                                            onClick={()=>{
                                                let {selected,text_check}=this.state;

                                                if(this.props.is_muti==true){
                                                    if(check){
                                                        text_check=text_check.replace(selected_show_img+item.id+",","");
                                                        let index=-1;
                                                        selected.forEach((a,x) => {
                                                            if(a.id==item.id) index=x;
                                                        });
                                                        if(index!=-1) selected.splice(index,1);
                                                        this.setState({selected:selected,text_check:text_check});
                                                    }else{
                                                        selected.push({
                                                            img:item.img,
                                                            id:item.id
                                                        });
                                                        text_check+=selected_show_img+item.id+",";
                                                        this.setState({selected:selected,text_check:text_check});
                                                    }
                                                }else{
                                                    if(check){
                                        ...
                                        <div className='rela'>
                                            ...
                                            <div className='col-6 textd'
                                                onClick={async()=>{
                                                    if (window.confirm(`Xóa hình ảnh này? (${item.id})`)) {
              ...
              <Modal.Footer>
                ...
                <Button variant="primary" onClick={()=>{
                    ...
                    if(this.props.is_muti==true){
                        this.props.fsReturn_media(selected)
                    }else{
