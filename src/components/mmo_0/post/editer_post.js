                  ...
                  <div>
                    {
                      data.img_sp.imgs_list.map((e,i)=>{
                        ...
                        ></i>
                        {((e.is_show_buy==undefined||e.is_show_buy==true)&&!a)&&<i className="fa-solid fa-cart-shopping abs hv" style={{bottom:"0px",left:"0px",color:"cornflowerblue"}}
                          onClick={()=>{
                            if(window.confirm("Ẩn nút mua hàng?")){
                              let {data}=this.state;
                              data.img_sp.imgs_list[i].is_show_buy=false
                              this.setState({ data: data })
                            }
                          }}
                        ></i>}
                        {((e.is_show_buy==false)&&!a)&&<i className="fa-solid fa-cart-shopping abs hv" style={{bottom:"0px",left:"0px",color:"#a7a5a5"}}
                          onClick={()=>{
                            if(window.confirm("Hiện nút mua hàng?")){
                              let {data}=this.state;
                              data.img_sp.imgs_list[i].is_show_buy=true
                              this.setState({ data: data })
                            }
                          }}
                        ></i>}
